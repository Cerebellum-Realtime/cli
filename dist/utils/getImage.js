...
const isValidECRImageURI = (uri) => {
    const ecrRegex = /^([a-zA-Z0-9\-_]+(?:\.[a-zA-Z0-9\-_]+)*\/)?([a-zA-Z0-9\-_]+\/[a-zA-Z0-9\-_]+):([a-zA-Z0-9\-_\.]+)$/;
    ...
    while (true) {
        try {
            ...
            if (response.toLowerCase() === "y") {
                ...
                const imageURI = await cli.prompt("Please enter ECR image URI *including the tag*");
                if (isValidECRImageURI(imageURI)) {
                    console.log("🧠 ECR image URI received!");
                    return imageURI;
                }
                else {
                    console.log("Invalid ECR image URI. Please try again. Make sure you remember to include the tag.");
                }
            }
            else {
